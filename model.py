...
class VariationalAutoencoder(tf.keras.Model):
    ...
    def call(self, x, training=False):
        ...
        # input_enc_combiners = tf.TensorArray(tf.float32, size=1, clear_after_read=False, infer_shape=False)
        # idx_enc = 0
        # for cell in self.enc_tower:
        #     if cell.cell_type == 'combiner_enc':
        #         print(f"tf.shape(x): {tf.shape(x)}")
        #         input_enc_combiners = input_enc_combiners.write(idx_enc, x)
        ...
        for cell in self.dec_tower:
            if cell.cell_type == 'combiner_dec':
                if idx_dec > 0: # This calculate log_p and log_q for KL divergence
                    ...
                    # ftr = enc_combiners[-idx_dec](input_enc_combiners.read(0), x)
                    mu_and_log_sigma_q = self.enc_mu_log_sig[idx_dec](ftr)
